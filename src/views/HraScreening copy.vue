        ...
        <t-card :bordered="true" class="mb">
          ...
          <div v-else class="form-grid">
            ...
            <div class="grid-3">
              ...
              <div>
                <label>依赖组 ID</label>
                <t-input v-model="aForm.dependence_group_id" placeholder="例如 RESTORE-VG01" />
              </div>
            </div>
...
import { calcCScreen, calcAScreen, listTasks, listScenarios } from '../api/hra'
...
async function onCalcC(){
  ...
  try{
    ...
    summary.value = [
      ...
      `HEP_final = ${resp.hepFinal}`,
      (resp.hepLb!=null && resp.hepUb!=null) ? `区间 = [${resp.hepLb}, ${resp.hepUb}], EF≈${resp.ef}` : ''
    ].filter(Boolean).join('\n')
...
async function onCalcA(){
  ...
  try{
    ...
    summary.value = [
      `BHEP = ${resp.bhep}`,
      `RF_fail = ${resp.rfFail}`,
      `RadFactor = ${resp.radFactor ?? '—'}`,
      `HEP_final = ${resp.hepFinal}`,
      (resp.hepLb!=null && resp.hepUb!=null) ? `区间 = [${resp.hepLb}, ${resp.hepUb}], EF≈${resp.ef}` : ''
    ].filter(Boolean).join('\n')
    resultTaskName.value = (t as any)?.taskName ?? (t as any)?.name
  }catch(e:any){
    console.error(e); MessagePlugin.error(`计算 A 类失败：${e?.message ?? ''}`)
  }
}
